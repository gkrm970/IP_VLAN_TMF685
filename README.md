...
Backend, JSON based web API based on OpenAPI: http://localhost/tmf685/v1

Automatic interactive documentation with Swagger UI (from the OpenAPI backend): http://localhost/tmf685/v1/docs

Prometheus Metrics: http://localhost/tmf685/v1/metrics

Code Coverage: http://localhost/tmf685/v1/coverage
...
In that case, you will need to use a fake local domain (`dev.tmf685.plt.tinaa.ocp01.osc.tac.net`) and make your computer think that the domain is is served by the custom IP (e.g. `192.168.99.150`).

If you used the default CORS enabled domains, `dev.tmf685.plt.tinaa.ocp01.osc.tac.net` was configured to be allowed. If you want a custom one, you need to add it to the list in the variable `BACKEND_CORS_ORIGINS` in the `.env` file.

* Open your `hosts` file with administrative privileges using a text editor:
  * **Note for Windows**: If you are in Windows, open the main Windows menu, search for "notepad", right click on it, and select the option "open as Administrator" or similar. Then click the "File" menu, "Open file", go to the directory `c:\Windows\System32\Drivers\etc\`, select the option to show "All files" instead of only "Text (.txt) files", and open the `hosts` file.
  * **Note for Mac and Linux**: Your `hosts` file is probably located at `/etc/hosts`, you can edit it in a terminal running `sudo nano /etc/hosts`.

* Additional to the contents it might have, add a new line with the custom IP (e.g. `192.168.99.150`) a space character, and your fake local domain: `dev.tmf685.plt.tinaa.ocp01.osc.tac.net`.

The new line might look like:

```
192.168.99.100    dev.tmf685.plt.tinaa.ocp01.osc.tac.net
```

* Save the file.
  * **Note for Windows**: Make sure you save the file as "All files", without an extension of `.txt`. By default, Windows tries to add the extension. Make sure the file is saved as is, without extension.

...that will make your computer think that the fake local domain is served by that custom IP, and when you open that URL in your browser, it will talk directly to your locally running server when it is asked to go to `dev.tmf685.plt.tinaa.ocp01.osc.tac.net` and think that it is a remote server while it is actually running in your computer.

To configure it in your stack, follow the section **Change the development "domain"** below, using the domain `dev.tmf685.plt.tinaa.ocp01.osc.tac.net`.

After performing those steps you should be able to open: http://dev.tmf685.plt.tinaa.ocp01.osc.tac.net and it will be server by your stack in `localhost`.

Check all the corresponding available URLs in the section at the end.

### Change the development "domain"

If you need to use your local stack with a different domain than `localhost`, you need to make sure the domain you use points to the IP where your stack is set up. See the different ways to achieve that in the sections above (i.e. using Docker Toolbox with `local.dockertoolbox.tiangolo.com`, using `localhost.tiangolo.com` or using `dev.tmf685.plt.tinaa.ocp01.osc.tac.net`).
...
you can re-start your stack with:
...
```bash
STACK_NAME=tmf685-tinaa-tlabs-ca sh ./scripts/deploy.sh
```
...
services:
  db:
    ...
    deploy:
      placement:
        constraints:
          - node.labels.tmf685-plt-tinaa-ocp01-osc-tac-net.app-db-data == true
```

**Note**: The `${STACK_NAME}` means "use the environment variable `STACK_NAME`, but if it is not set, show an error ``".
...
```bash
docker node update --label-add tmf685-plt-tinaa-ocp01-osc-tac-net.app-db-data=true dog.example.com
```

* Then you need to do the same for each stack version you have. For example, for staging you could do:

```bash
docker node update --label-add tmf685-tinaa-tlabs-ca.app-db-data=true cat.example.com
```
...
* Set these environment variables:
  * `DOMAIN=tmf685.plt.tinaa.ocp01.osc.tac.net`
  * `TRAEFIK_TAG=tmf685.plt.tinaa.ocp01.osc.tac.net`
  * `STACK_NAME=tmf685-plt-tinaa-ocp01-osc-tac-net`
  * `TAG=prod`
...
Backend: https://tmf685.plt.tinaa.ocp01.osc.tac.net/api/

Automatic Interactive Docs (Swagger UI): https://tmf685.plt.tinaa.ocp01.osc.tac.net/docs

Automatic Alternative Docs (ReDoc): https://tmf685.plt.tinaa.ocp01.osc.tac.net/redoc


PGAdmin: https://pgadmin.tmf685.plt.tinaa.ocp01.osc.tac.net
...
Backend: https://tmf685.tinaa.tlabs.ca/api/

Automatic Interactive Docs (Swagger UI): https://tmf685.tinaa.tlabs.ca/docs

Automatic Alternative Docs (ReDoc): https://tmf685.tinaa.tlabs.ca/redoc


PGAdmin: https://pgadmin.tmf685.tinaa.tlabs.ca
...
Backend: http://dev.tmf685.plt.tinaa.ocp01.osc.tac.net/api/

Automatic Interactive Docs (Swagger UI): https://dev.tmf685.plt.tinaa.ocp01.osc.tac.net/docs

Automatic Alternative Docs (ReDoc): https://dev.tmf685.plt.tinaa.ocp01.osc.tac.net/redoc


PGAdmin: http://dev.tmf685.plt.tinaa.ocp01.osc.tac.net:5050



Traefik UI: http://dev.tmf685.plt.tinaa.ocp01.osc.tac.net:8090
...
cookiecutter https://gitlab.tinaa.tlabs.ca/tinaa/platform/utils/tinaa-cookiecutter.git
```
...
```console
$ cookiecutter --config-file ./cookiecutter-config-file.yml --output-dir ../project-copy https://gitlab.tinaa.tlabs.ca/tinaa/platform/utils/tinaa-cookiecutter.git
```
