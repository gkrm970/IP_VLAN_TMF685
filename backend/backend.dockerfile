...
RUN apk update && \
    apk add --no-cache \
postgresql-dev \
python3-dev autoconf automake g++ make \
      libffi libffi-dev  libxml2-dev  libxslt-dev libressl-dev ca-certificates && \
...
RUN pip install --no-cache-dir --upgrade pip && \
    pip install --no-cache-dir "cryptography==3.3.2" poetry pipenv && \
